...
nonlandsToRemove = [item.lower() for item in nonlandsToRemove]

landsToRemove =['fabled passage', 'terramorphic expanse', 'evolving wilds']
landsToRemove = [item.lower() for item in landsToRemove]
...
def create_deck_list(commander_name):
    ...
    while nonlandCount < 61:
        ...
        for card in filtered_data:
            card_name = card['name']
            if card_name.lower() not in nonlandsToRemove:
